...
function deletea(event) {
    var questionsId = $(event.target).attr("questionsid");
    $.ajax({
        type: "POST", //请求方式
        url: "/exam/backExaminationController/deleteQuestions",
        dataType: "JSON",
        data: {
            list: questionsId
        }
    ...
    $.ajax({
        ...
        success: function (data) {
            for (var i = 0; i < data.length; i++) {
                ...
                $("#que").append("<div class='nav-content' id='bigQuestion" + i + " 'questionId = "+data[i].id+"><div class='que-type' index=''><span class='type-name'>"+titleStra[i]+"</span><span class='index'></span>"+ titleStrs[i]+"<button type='button' class='btn btn-danger delete' questionsId = "+data[i].id+" onclick='deletea(event)'>删除</button></div></div>");
            }
            //循环出大题 将大题放到对应的小题中
